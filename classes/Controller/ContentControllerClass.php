...
class ContentControllerClass extends BaseControllerClass
{
    ...
    public function importMedia()
    {
        ...
        if (!is_dir(dirname($userDirPath))) {
            ...
            chmod(dirname($userDirPath), 0777);
        }
        if (!is_dir($userDirPath)) {
            mkdir($userDirPath);
            chmod($userDirPath, 0777);
        }
    ...
    public function getMediaList($type, $userId = 0)
    {
        ...
        if($type == 'input'){
            $files = glob($this->getPublicPath('input_dir').'/{*.mp4,*.flv,*.mov}', GLOB_BRACE);
            $data = Array();
            foreach ($files as $file) {
                ...
                $properties = $this->getVideoProperties($file);



                $sub_data = array(
                    'id' => basename($file, '.'.$ext),
                    'title' => strip_tags(basename($file, '.'.$ext)),
                    'ext' => $ext,
                    'file_size' => self::sizeFormat($fileSize),
                    'time_stump' => time(),
                    'datetime' => date('m.d.Y H:i:s', time()),
                    'duration_time' => !empty($properties['duration_ms'])
                        ? self::secondsToTime($properties['duration_ms'] / 1000)
                        : 0,
                    'url' => $this->config['base_url'] . $this->config[$type . '_dir'] . basename($file, '.'.$ext) . '.' . $ext,
                    // 'url'] .=> $user['id'] . DIRECTORY_SEPARATOR . $item['id'] . '.' . $item['ext'],


                );

                $sub_data = array_merge($sub_data, $properties);
                $data[] = $sub_data;
            }
            return array(       
                'success' => true,
            ...
            //         "file_size"=> 13663417,
            //         "height"=>1080,
            //         "id"=>"1565765251_5d53ae83424e1",
            //         "streams"=>2,
            //         "time_stump"=>1565765255,
            //         "title"=>"r_AUTO_ad102_2019-08-08_09-05-32_54242",
            //         "type"=> "video",
            //         "width"=>1920 
            //     ),
            //     array(
            ...
            $data = scandir($this->getPublicPath('output_dir'));
            $data = [];
            // $data = array(
            //     array(
            //         "duration_ms"=>15004,
        ...
        $files = glob($this->getPublicPath('input_dir').'/{*.mp4,*.flv,*.mov}', GLOB_BRACE);
            ...
            foreach ($files as $file) {
                ...
                if($sub_data['id'] == $itemId){
                    ...
                    $filePath = $this->getPublicPath($type . '_dir');            
                    $filePath .= DIRECTORY_SEPARATOR . $itemId . '.' . $ext;
                    $filePath1 = $this->getPublicPath($type . '_dir');            
                    $filePath1 .= DIRECTORY_SEPARATOR . $data['title'] . '.' . $ext;
                    rename($filePath, $filePath1);
                    // basename($file, '.'.$ext) = $data;
                    // rename( $sub_data['url'], this->config['base_url'] . $this->config[$type . '_dir'] . $data . '.' . $ext);
                    return array(
                        'success' => true,
                        'data' => $sub_data
                    );
