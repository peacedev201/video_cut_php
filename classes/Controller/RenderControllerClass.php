...
class RenderControllerClass extends BaseControllerClass
{
     ...
     * @return array
     */
    public function render($title,$code, $opts, $data)
    {
        // print_r($opts); exit;
        if (!is_array($data)) {
        ...
        $tmpDirPath = $this->getPublicPath('tmp_dir', $user['id']);
        $inputDirPath = $this->getPublicPath('input_dir');
        $outputDirPath = $this->getPublicPath('output_dir', $code);

        if (!is_dir(dirname($tmpDirPath))) {
            mkdir(dirname($tmpDirPath));
            chmod(dirname($tmpDirPath), 0777);
        }
        if (!is_dir($tmpDirPath)) {
            mkdir($tmpDirPath);
            chmod($tmpDirPath, 0777);
        ...
        if (!is_dir(dirname($outputDirPath))) {
            ...
            chmod(dirname($outputDirPath), 0777);
        }
        if (!is_dir($outputDirPath)) {
            mkdir($outputDirPath);
            chmod($outputDirPath, 0777);
        ...
        foreach ($data as $index => $input) {
            ...
            $filePath = $this->getInputMediaFilePath('input', $media);
        ...
        if (!empty($options['longtext']) && $imagesCount && $videoInputLength) {
            ...
            $totalDur = $c * 20000;
            $imagesDuration = ceil(($totalDur - $videoInputLength) / $imagesCount);
            $imagesDuration = max(3000, $imagesDuration);
        }
        ...
        foreach ($inputData as $index => $input) {
            ...
            if ($input['type'] == 'image') {
                ...
                $dur = $imagesDuration ? $imagesDuration / 1000 : $input['duration'];
        ...
        foreach ($inputData as $index => $input) {
            ...
            if (!empty($input['time'])) {
                ...
                $dur = $input['type'] == 'image' && $imagesDuration ? $imagesDuration : intval($input['duration']);
        ...
        if ($audioBackgroundIndex > -1) {
            ...
            //Fade audio
            $cmdFilters .= ' \\' . PHP_EOL . "[{$audioOutName}]" . 'afade=t=out:st=' . ( $totalDuration - 6 ) . ':d=6[audio_with_fade];';
